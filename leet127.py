...
注意，startWord和endWord可能不在字典里！
按照字典里的单词BFS.超时。。(Solution1)

换个思路，每次改变单词的一位，BFS. 但是由于Python中，字符串是不可变对象，
因此变换单词是耗时很久(Solution2).超时。。 sigh...
...
class Solution1:
    ...
    def ladderLength(self, beginWord, endWord, wordDict):
        self.queue=[beginWord,'/']
        self.dict=list(wordDict)
        self.l=len(wordDict)
        self.endWord=endWord
        self.ans=0
        self.used=[False for i in range(self.l)]
        i=0
        k=1
    ...
    def bfs(self, word, kth):
        if self.diffone(word, self.endWord):
            self.ans=kth+1
            return
        # 执行3个样例， 12.352s
        # for x in self.dict:
        #     if self.diffone(word, x) and (x not in self.queue):
        #         self.queue.append(x)
        
        # 执行3个样例，3.477s
        for x in self.dict:
            if self.diffone(word, x):
                self.queue.append(x)
                self.dict.remove(x)

        # 执行3个样例，3.976s
        # for i in xrange(self.l):
        #     if (not self.used[i]) and self.diffone(word, self.dict[i]):
        #         self.queue.append(self.dict[i])
        #         self.used[i]=True
        ...
        while i<len(queue):
            ...
            for j in range(ll):
                for x in "abcdefghijklmnopqrstuvwxyz":
                    if queue[i][j]!=x:
                        newword=queue[i][:j]+x+queue[i][j+1:]
                        if newword==endWord:
                            return k+1
                        if newword in wordDict and newword not in dic:
